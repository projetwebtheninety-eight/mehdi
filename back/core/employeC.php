<?PHP
include "E:\LOGICIELS\wamp64\www\laCaveAdmin\config.php";
class CommandeC {
function afficherEmploye ($commande){
    ...
    function afficherEmployestri(){
        //$sql="SElECT * From employe e inner join formationphp.employe a on e.cin= a.cin";
        $sql="SElECT * From commande order by reference";
        $db = config::getConnexion();
        try{
            $liste=$db->query($sql);
            return $liste;
	...
	function modifierEmploye($commande,$reference){
		$sql="UPDATE commande SET reference=:reference, nom=:nom,mail=:mail,telephone=:telephone,date=:date,adresse=:adresse WHERE reference=:reference";
